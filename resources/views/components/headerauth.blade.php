...
<header class="flex flex-row justify-between w-full">
    <div class="flex flex-row items-center gap-5">
        {{ $slot }}  
        <form action="{{ route('auth.search') }}" class="flex justify-center items-center gap-1">
            @csrf
            <input name="query" placeholder="Search..." class="border-[1px] rounded-full border-gray-300 h-full px-5" type="text">
